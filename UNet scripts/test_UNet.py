...
model.load_state_dict(torch.load(f'model-{dataset}.pth', map_location=torch.device('cpu')))
model = model.cuda()
...
if dataset == 'PH2':
    from ph2 import PH2
    ts = PH2('test', test_transforms)
if dataset[0:11] == 'PH2-patches':
    from ph2 import PH2
    ts = PH2('train', 'patches', testP_transforms)
if dataset == 'EVICAN':
...
#    print('Y:',type(Y), Y.dtype, Y.shape)
    ...
    if numb < 300:
        ...
        plt.show()
        plt.close(fig)
        fig.savefig(f'{dataset}_train_images/{dataset}_img{numb}.png',bbox_inches='tight', dpi=150)
